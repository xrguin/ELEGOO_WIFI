...
alignment_heading_tolerance = 10; % Alignment phase tolerance (degrees) - Tightened for better alignment
...
h_main_fig = figure('Position', [50, 50, 2200, 1000], 'Name', 'APF Dual Robot Simulation');
sgtitle('APF Dual Robot Mutual Avoidance', 'FontSize', 16);

% Left plot: World View (spans two rows)
ax_world = subplot(2,4,[1,5]);
...
% Second column top plot: Robot 1 Forces
ax_force_r1 = subplot(2,4,2);
...
% Second column bottom plot: Robot 2 Forces
ax_force_r2 = subplot(2,4,6);
...
% Third column top plot: Robot 1 Heading
ax_heading_r1 = subplot(2,4,3);
hold(ax_heading_r1, 'on'); grid on;
title(ax_heading_r1, 'Robot 1 Heading Control');
xlabel(ax_heading_r1, 'Time (s)'); ylabel(ax_heading_r1, 'Heading (degrees)');
ylim(ax_heading_r1, [-180, 180]);

% Third column bottom plot: Robot 2 Heading
ax_heading_r2 = subplot(2,4,7);
hold(ax_heading_r2, 'on'); grid on;
title(ax_heading_r2, 'Robot 2 Heading Control');
xlabel(ax_heading_r2, 'Time (s)'); ylabel(ax_heading_r2, 'Heading (degrees)');
ylim(ax_heading_r2, [-180, 180]);

% Fourth column top plot: Robot 1 Motor Speeds
ax_motor_r1 = subplot(2,4,4);
hold(ax_motor_r1, 'on'); grid on;
title(ax_motor_r1, 'Robot 1 Motor Speeds');
xlabel(ax_motor_r1, 'Time (s)'); ylabel(ax_motor_r1, 'Speed (PWM)');
ylim(ax_motor_r1, [-255, 255]);

% Fourth column bottom plot: Robot 2 Motor Speeds
ax_motor_r2 = subplot(2,4,8);
hold(ax_motor_r2, 'on'); grid on;
title(ax_motor_r2, 'Robot 2 Motor Speeds');
xlabel(ax_motor_r2, 'Time (s)'); ylabel(ax_motor_r2, 'Speed (PWM)');
ylim(ax_motor_r2, [-255, 255]);
...
% -- Motor Speed Plot Handles
h_r1_left_motor = plot(ax_motor_r1, 0, 0, 'b-', 'LineWidth', 2, 'DisplayName', 'Left Motor');
h_r1_right_motor = plot(ax_motor_r1, 0, 0, 'r-', 'LineWidth', 2, 'DisplayName', 'Right Motor');
legend(ax_motor_r1, 'Location', 'best');
h_r2_left_motor = plot(ax_motor_r2, 0, 0, 'b-', 'LineWidth', 2, 'DisplayName', 'Left Motor');
h_r2_right_motor = plot(ax_motor_r2, 0, 0, 'r-', 'LineWidth', 2, 'DisplayName', 'Right Motor');
legend(ax_motor_r2, 'Location', 'best');
...
r1_motor_time_data = []; r1_left_motor_data = []; r1_right_motor_data = []; % Motor speed data
r1_start_time = tic; % Track time from start for all stages
...
r2_motor_time_data = []; r2_left_motor_data = []; r2_right_motor_data = []; % Motor speed data
r2_start_time = tic; % Track time from start for all stages
...
try
    while true
        ...
        % -- Process Robot 1 Data FIRST (to match control execution order)
        ...
        % -- Process Robot 2 Data SECOND (to match control execution order)
        ...
        if r1_goal_reached && r2_goal_reached, disp('Both robots reached goals!'); send_robot_command(robot1_ip, 'S'); send_robot_command(robot2_ip, 'S'); break; end
           ...
           % -- ROBOT 1 CONTROL (Execute first to get fresh position)
        ...
        % -- ROBOT 2 CONTROL (Execute second, using fresh r1_pos from current iteration)
        ...
        % -- SEND COMMANDS AND RECORD MOTOR SPEEDS
        % Robot 1 motor speeds (track for all stages, not just APF)
        current_time_r1 = toc(r1_start_time);
        r1_motor_time_data(end+1) = current_time_r1;
        r1_left_motor_data(end+1) = r1_left;
        r1_right_motor_data(end+1) = r1_right;
        
        % Robot 2 motor speeds (track for all stages, not just APF)
        current_time_r2 = toc(r2_start_time);
        r2_motor_time_data(end+1) = current_time_r2;
        r2_left_motor_data(end+1) = r2_left;
        r2_right_motor_data(end+1) = r2_right;
        ...
        % -- Motor Speed Plot Updates (for ALL stages)
        set(h_r1_left_motor, 'XData', r1_motor_time_data, 'YData', r1_left_motor_data);
        set(h_r1_right_motor, 'XData', r1_motor_time_data, 'YData', r1_right_motor_data);
        set(h_r2_left_motor, 'XData', r2_motor_time_data, 'YData', r2_left_motor_data);
        set(h_r2_right_motor, 'XData', r2_motor_time_data, 'YData', r2_right_motor_data);
...
function [left_speed, right_speed, state, integral_w, prev_error_w, goal_reached, alignment_timer, apf_desired_heading, heading_error, integral_d, prev_error_d, F_att_mag, F_rep_mag, new_prev_obstacle_dist, new_min_dist_reached] = ...
    ...
    switch state
        case {'INITIAL_APPROACH', 'ALIGNMENT_PREP', 'ALIGNING', 'WAITING_FOR_PARTNER'}
            ...
            if strcmp(state, 'INITIAL_APPROACH')
                % Check for obstacle (other robot) and apply APF if needed
                current_pos_2d = [current_pos_3d(1); current_pos_3d(3)];
                start_pos_2d = [start_pos(1); start_pos(3)];
                obstacle_pos_2d = [obstacle_pos_3d(1); obstacle_pos_3d(3)];
                
                obstacle_dist = norm(obstacle_pos_2d - current_pos_2d);
                
                if obstacle_dist < detection_radius
                    % Use APF for navigation with obstacle avoidance
                    [F_att, F_rep, F_combined] = calculate_apf_forces(current_pos_2d, start_pos_2d, obstacle_pos_2d, detection_radius, attraction_factor, repulsion_factor, false);
                    
                    if norm(F_combined) > 0.01
                        target_angle = rad2deg(atan2(F_combined(1), F_combined(2)));
                    else
                        % Fallback to direct heading if forces cancel out
                        target_vec = [start_pos(1) - current_pos_3d(1), start_pos(3) - current_pos_3d(3)];
                        target_angle = rad2deg(atan2(target_vec(1), target_vec(2)));
                    end
                    fprintf('R%d State: %s (APF) | Obstacle at %.1f mm | ', robot_num, state, obstacle_dist);
                else
                    % No obstacle nearby, use direct heading
                    target_vec = [start_pos(1) - current_pos_3d(1), start_pos(3) - current_pos_3d(3)];
                    target_angle = rad2deg(atan2(target_vec(1), target_vec(2)));
                ...
                fprintf('Dist to Start: %.1f mm | Fwd (PID): %.1f | Turn: %.1f\n', dist_to_start, fwd_speed, turn_speed);
        ...
        case 'APF_CONTROL'
            ...
            [turn_adjust, new_integral_w, new_prev_error_w] = pid_control(heading_error, dt, Kp_w, Ki_w, Kd_w, integral_w, prev_error_w);
            integral_w = new_integral_w;  % Update the PID states
            prev_error_w = new_prev_error_w;  % Update the PID states
    ...
    % ELEGOO N:4 command only accepts positive values (0-255)
    % Negative values will cause motors to stop
    % Clamp to valid range: 0 to 80 (max forward speed limit)
    left_speed = round(min(max(left_speed, 0), 80));
    right_speed = round(min(max(right_speed, 0), 80));
end
